  ...
  class LewdSite extends TinyIB {
    constructor(prot, dm) {
      ...
      this.JsonBuilder = class LewdSiteJsonBuilder {
        ...
        getPostHTML(i) {
          const post = this._posts[i + 1];
          const className = [
            'post',
            typeof post.parent_id === 'undefined' || post.parent_id === 0 || post.parent_id === null ? 'post_op' : 'post_reply',
                  ...
                  <td id=${`reply${post.id}`} class="reply">
                    <div class="${className}">
                      ...
                      <label>
                        ...
                        <time class="post__date" datetime="${new Date(post.created_at).toISOString()}">${date}</time>
                      </label>
        ...
        *bannedPostsData() {}
        ...
        _formatFileDimensions(file) {
          const dimensions = [this._formatFileSize(file.size)];
          ...
          if (file.length !== null) {
            dimensions.push(this._formatDuration(file.length));
          }
        ...
        _markup(post, nodes) {
          return nodes.map((node) => this._markupNode(post, node));
        }

        _markupNode(post, node) {
          switch (node.type) {
            case 'text':
              return node.text;
            ...
            case 'link':
              return `<a href="${node.url}" target="_blank" rel="ugc">${node.text}</a>`;

            case 'reflink':
              const url = `/${post.slug}/res/${post.parentId || post.id}.html#${node.postID}`;

              return `<a href="${url}" rel="ugc">&gt;&gt;${node.postID}</a>`;
            ...
            case 'dice':
              ...
              return `<span class="dice">${textContent}</span>`;
            ...
            case 'style':
              ...
              switch (node.style) {
                ...
                case 'size':
                  return `<span style="font-size: ${node.value}px;">${content}</span>`;

                case 'color':
                  return `<span style="color: ${node.value};">${content}</span>`;
